...
LETS_ENCRYPT="n"
read -rp "(!) Enter the domain name for your bitwarden instance (ex. bitwarden.company.com): " DOMAIN
...
if [ "$DOMAIN" != "localhost" ]
then
  read -rp "(!) Do you want to use Let's Encrypt to generate a free SSL certificate? (y/n): " LETS_ENCRYPT

  if [ "$LETS_ENCRYPT" == "y" ]
  then
    read -rp "(!) Enter your email address (Let's Encrypt will send you certificate expiration reminders): " EMAIL
